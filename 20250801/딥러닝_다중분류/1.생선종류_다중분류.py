...
model = Sequential()
model.add(Dense(units=30, input_dim=5, activation='relu')) # 히든층
model.add(Dense(units=7, activation='softmax')) # 출력층

model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

model.fit(train_scaled, train_target, epochs=1000, verbose=1)

score = model.evaluate(test_scaled, test_target)
print('성능평가 : ', score[1])

model.save('fish_bestmodel.h5')